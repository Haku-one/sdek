...
class CdekDeliveryPlugin {
    ...
    public function init_cdek_shipping() {
        if (!class_exists('WC_Cdek_Shipping_Method')) {
            include_once 'includes/class-wc-cdek-shipping-method.php';
        }
    }
    ...
    public function activate_plugin() {
        ...
        if (!get_option('cdek_plugin_version')) {
            ...
            add_option('cdek_sender_city', '44');
        }
    }
    
    public function load_blocks_integration() {
        if (class_exists('Automattic\WooCommerce\Blocks\Integrations\IntegrationInterface')) {
            include_once 'includes/class-wc-blocks-integration.php';
        }
    }
}
...
class CdekAPI {
    ...
    public function get_delivery_points($address) {
        ...
        $response = wp_remote_get($this->base_url . '/deliverypoints', array(
            'headers' => array(
    ...
    public function calculate_delivery_cost($from_location, $to_location, $packages) {
        $token = $this->get_auth_token();
        if (!$token) {
            return false;
        }
