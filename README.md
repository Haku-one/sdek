...
### ⚠️ ВАЖНО: Устранение ошибки 404

Если вы видите ошибку "Failed to load resource: the server responded with a status of 404" для `cdek-delivery.js`, это означает, что файл не найден по указанному пути.

### Решение ошибки 404:

1. **Определите, где находятся ваши JS файлы:**
   - В папке `/js/` темы: `/wp-content/themes/ваша-тема/js/`
   - В корне темы: `/wp-content/themes/ваша-тема/`
   - В другом месте

2. **Замените функцию `cdek_enqueue_scripts()` в functions.php:**
   Удалите старую функцию и используйте код из файла `functions-php-fix.php`

3. **Альтернативный способ (если автопоиск не работает):**
   Раскомментируйте функцию `cdek_enqueue_scripts_manual()` и укажите прямые пути к файлам

4. **Проверьте права доступа к файлам:**
   Убедитесь, что файлы доступны для чтения веб-сервером
...
### Ошибка 404 (Not Found) для JS файлов
**Причина**: JavaScript файлы не найдены по указанному пути  
**Решение**: 
1. Проверьте, что файлы `cdek-delivery.js` и `cdek-cart.js` загружены на сервер
2. Замените функцию подключения скриптов на код из `functions-php-fix.php`
3. Или укажите прямые пути к файлам в альтернативной функции
