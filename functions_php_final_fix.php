<?php
// ФИНАЛЬНОЕ РЕШЕНИЕ - заполняем ВСЕ поля и скрываем всю секцию доставки
...
function add_discuss_delivery_and_fill_fields() {
    if (is_checkout()) {
        ...
        /* Скрываем всю секцию доставки при выборе обсуждения */
        .shipping-section-hidden .wc-block-checkout__shipping-option,
        .shipping-section-hidden .wp-block-woocommerce-checkout-shipping-methods-block,
        .shipping-section-hidden .wp-block-cdek-checkout-map-block,
        .shipping-section-hidden #cdek-map-container,
        .shipping-section-hidden .wc-block-components-shipping-rates-control,
        .shipping-section-hidden fieldset#shipping-option {
            display: none !important;
        }
        </style>
        
        <script>
        function forceSetFieldValue(element, value) {
            if (!element) return;
            ...
            // Убираем required и ошибки
            element.removeAttribute("required");
            element.setAttribute("aria-invalid", "false");
            element.classList.remove("wc-invalid", "has-error");
            ...
            console.log("✅ Поле заполнено:", element.id, "значением:", value);
        }
        
        function fillAllAddressFields() {
            console.log("🔥 ПРИНУДИТЕЛЬНО заполняем ВСЕ адресные поля...");
            
            // Список всех возможных селекторов для каждого поля
            const fieldMappings = {
                city: [
            ...
            // Заполняем city и state значением "М"
            ['city', 'state'].forEach(fieldType => {
                fieldMappings[fieldType].forEach(selector => {
                    const elements = document.querySelectorAll(selector);
                    elements.forEach(el => forceSetFieldValue(el, "М"));
                });
            });
            
            // Заполняем postcode значением "000000"
            fieldMappings.postcode.forEach(selector => {
                const elements = document.querySelectorAll(selector);
                elements.forEach(el => forceSetFieldValue(el, "000000"));
            });
        ...
        function addDiscussDeliveryButton() {
            ...
            if (shippingContainer && !document.getElementById("discuss-delivery-btn")) {
                ...
                button.addEventListener("click", function() {
                    ...
                    // СКРЫВАЕМ ВСЮ СЕКЦИЮ ДОСТАВКИ (как при самовывозе)
                    document.body.classList.add("shipping-section-hidden");
                    
                    // Скрываем все элементы доставки
                    const shippingSelectors = [
                        ...
                        ".wc-block-checkout__shipping-method-option:not(#discuss-delivery-btn)"
                    ];
                    ...
                    console.log("✅ Вся секция доставки скрыта (как при самовывозе)");
                });
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            setTimeout(fillAllAddressFields, 100);
            
            // Добавляем кнопку
            setTimeout(addDiscussDeliveryButton, 500);
            
            // Агрессивное заполнение полей каждые 200ms
            setInterval(fillAllAddressFields, 200);
            ...
            // Дополнительное заполнение при любых событиях
            ['focus', 'blur', 'change', 'input', 'click'].forEach(eventType => {
                document.addEventListener(eventType, fillAllAddressFields, true);
            });
        });
        </script>
        <?php
